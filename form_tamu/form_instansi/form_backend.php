...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['nama_lengkap'])) {
    if ($tamu_aktif) {
        ...
        if (empty($no_id_tamu) || !is_numeric($no_id_tamu)) {
            $errors[] = "No identitas harus berupa angka!";
        }
        ...
        $check_query = "SELECT COUNT(*) as count FROM data_tamu 
                       WHERE no_id = ? 
                       AND DATE(waktu_checkin) = CURDATE() 
                       AND status_aktif = 'aktif'";
        $check_stmt = mysqli_prepare($koneksi, $check_query);
        mysqli_stmt_bind_param($check_stmt, "s", $no_id_tamu);
